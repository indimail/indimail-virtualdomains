...
static const char *
resolveopenflags(u_int32_t flags)
{
    static char     buf[160];
    char            b2[80];
    strlcpy(buf, "", sizeof (buf));
    if (flags & DB_CREATE)
        flags &= ~DB_CREATE, strlcat(buf, "DB_CREATE ", sizeof (buf));
    if (flags & DB_EXCL)
        flags &= ~DB_EXCL, strlcat(buf, "DB_EXCL ", sizeof (buf));
    if (flags & DB_NOMMAP)
        flags &= ~DB_NOMMAP, strlcat(buf, "DB_NOMMAP ", sizeof (buf));
    if (flags & DB_RDONLY)
        flags &= ~DB_RDONLY, strlcat(buf, "DB_RDONLY ", sizeof (buf));
#if DB_AT_LEAST(4,1)
    if (flags & DB_AUTO_COMMIT)
        flags &= ~DB_AUTO_COMMIT, strlcat(buf, "DB_AUTO_COMMIT ", sizeof (buf));
#endif
...
get_txn_state(bfpath * bfp)
{
    ...
    if (txn != T_DISABLED && txn != T_ENABLED) {
        e_txn           probe; /*- if not set, probe for database environment */

        if (DEBUG_DATABASE(1))
            fprintf(dbgout, "probing \"%s\" and \"%s\" for environment...", bfp->dirname, bfp->filename);
 ...
 */
static int
DB_OPEN(DB * db, bfpath * bfp, const char *database, DBTYPE type, u_int32_t flags, int mode)
{
    int             ret;
    const char     *file;
    ...
    if (DEBUG_DATABASE(1) || getenv("BF_DEBUG_DB_OPEN"))
        fprintf(dbgout, "[pid %lu] DB->open(db=%p, file=%s, database=%s, " "type=%x, flags=%#lx=%s, mode=%#o) -> %d %s\n",
                (unsigned long) getpid(), (void *) db, file, database ? database : "NIL", type, (unsigned long) flags,
...
#if DB_AT_LEAST(4,1)
/*- translate BerkeleyDB flags bitfield for DB->set_flags method back to symbols */
static const char *
resolvesetflags(u_int32_t flags)
{
    static char     buf[160];
    char            b2[80];
    strlcpy(buf, "", sizeof (buf));
#if DB_EQUAL(4,1)
    if (flags & DB_CHKSUM_SHA1)
        flags &= ~DB_CHKSUM_SHA1, strlcat(buf, "DB_CHKSUM_SHA1 ", sizeof (buf));
#endif
#if DB_AT_LEAST(4,2)
    if (flags & DB_CHKSUM)
        flags &= ~DB_CHKSUM, strlcat(buf, "DB_CHKSUM ", sizeof (buf));
#endif
    snprintf(b2, sizeof (b2), "%#lx", (unsigned long) flags);
    if (flags)
        strlcat(buf, b2, sizeof (buf));
 ...
 */
int
subr_db_lock(int fd, int cmd, short int type)
{
    struct flock    lock;
...
int
db_lock(void *vhandle, int open_mode)
{
    int             e = 0;
    dbh_t          *handle = vhandle;
    ...
    /*- try fcntl lock */
    handle->locked = false;
    if (subr_db_lock(handle->fd, F_SETLK, (short int) (open_mode == DS_READ ? F_RDLCK : F_WRLCK))) {
        e = errno;
        db_close(handle);
        errno = e;
        if (errno == EACCES)
...
static bool
xstat(const char *path, struct stat *st)
{
    int             e = stat(path, st);
    if (e) {
 ...
 */
static bool
check_path(bfpath * bfp)
{
    struct stat     stdn, stbh;

   /*- 
    * Only bogofilter (with transactions) has
    * to worry about multiple environments
    */
    ...
    */
    if (xstat(bfp->dirname, &stdn))
        return true;
    if (xstat(bogohome, &stbh))
        return true;

    if (stdn.st_dev == stbh.st_dev && stdn.st_ino == stbh.st_ino)
        return true;
    else {
        fprintf(stderr,
 ...
 */
static bool
check_fsize_limit(int fd, uint32_t pagesize)
{
    ...
    if (rl.rlim_cur != (rlim_t) RLIM_INFINITY) {
        ...
        */
        if ((off_t) (rl.rlim_cur / pagesize) - st.st_size / (long) pagesize < 16) {
            ...
            ok = false;
        } else if ((off_t) (rl.rlim_cur >> 20) - (st.st_size >> 20) < 2) {
            fPrinted = true;
 ...
 *
 * dbp - Berkeley DB pointer 
 * wanted - if set, try harder to get the page
 *          size, even if it requires to read the whole database
 */
static          uint32_t
get_psize(DB * dbp, bool wanted)
{
...
#else
    snprintf(v, sizeof (v), "BerkeleyDB (%d.%d.%d)", DB_VERSION_MAJOR, DB_VERSION_MINOR, DB_VERSION_PATCH);
#endif
 ...
 */
void           *
db_open(void *vhandle, bfpath * bfp, dbmode_t open_mode)
{
    int             ret;
    int             is_swapped;
  ...
  /*- retry when locking failed */
    for (idx = 0; idx < COUNTOF(retryflags); idx += 1) {
        ...
        if ((ret = db_create(&dbp, dbe, 0)) != 0) {
            print_error(__FILE__, __LINE__, "(db) db_create, err: %d, %s", ret, db_strerror(ret));
            goto open_err;
        }
      ...
      retry_db_open:
        ...
        if (err) {
            ...
            if (DEBUG_DATABASE(0))
                print_error(__FILE__, __LINE__, "DB->open(%s) - actually %s, directory %s, err %s", handle->name, db_file,
                            env->directory, db_strerror(ret));
        ...
        if (ret != 0) {
            print_error(__FILE__, __LINE__, "DB->get_byteswapped: %s", db_strerror(ret));
            db_close(handle);
        ...
        if (ret != 0) {
            print_error(__FILE__, __LINE__, "DB->fd: %s", db_strerror(ret));
            db_close(handle);
            return NULL;        /* handle already freed, ok to return */
        }
...
db_delete(void *vhandle, const dbv_t * token)
{
    ...
    if (ret != 0 && ret != DB_NOTFOUND) {
        print_error(__FILE__, __LINE__, "DB->del('%.*s'), err: %d, %s", CLAMP_INT_MAX(db_key.size), (const char *) db_key.data, ret,
                    db_strerror(ret));
        exit(EX_ERROR);
    }
...
int
db_get_dbvalue(void *vhandle, const dbv_t * token, /*@out@ */ dbv_t * val)
{
    int             ret = 0;
    int             rmw_flag;
    DBT             db_key;
    ...
    case DB_LOCK_DEADLOCK:
        ...
        break;
    default:
        print_error(__FILE__, __LINE__, "(db) DB->get(TXN=%lu,  '%.*s' ), err: %d, %s", (unsigned long) handle->txn,
...
int
db_set_dbvalue(void *vhandle, const dbv_t * token, const dbv_t * val)
{
    int             ret;
    ...
    if (ret != 0) {
        print_error(__FILE__, __LINE__, "db_set_dbvalue( '%.*s' ), err: %d, %s", CLAMP_INT_MAX(token->leng), (char *) token->data,
                    ret, db_strerror(ret));
        exit(EX_ERROR);
    }
 ...
 */
static          uint32_t
db_get_flags(DB * dbp, uint32_t test)
{
    int             ret;
    uint32_t        flags;
    ...
    if (ret) {
        print_error(__FILE__, __LINE__, "DB->get_flags returned error: %s", db_strerror(ret));
        mask = 0;
    } else {
        if (flags & test)
            mask &= ~DB_NOSYNC;
...
db_close(void *vhandle)
{
    ...
    if (ret)
        print_error(__FILE__, __LINE__, "DB->close error: %s", db_strerror(ret));
...
db_foreach(void *vhandle, db_foreach_t hook, void *userdata)
{
...
#if DB_AT_LEAST(4,6)
    for (i = dbcp->get(dbcp, &key, &data, DB_FIRST); i == 0; i = dbcp->get(dbcp, &key, &data, DB_NEXT))
#else
    for (i = dbcp->c_get(dbcp, &key, &data, DB_FIRST); i == 0; i = dbcp->c_get(dbcp, &key, &data, DB_NEXT))
#endif
    {
        int             rc;
    ...
    case DB_NOTFOUND:
        ...
        break;
    default:
        print_error(__FILE__, __LINE__, "(c_get): %s", db_strerror(i));
        eflag = true;
        break;
    }

#if DB_AT_LEAST(4,6)
    if ((i = dbcp->close(dbcp)))
#else
    if ((i = dbcp->c_close(dbcp)))
#endif
    {
        print_error(__FILE__, __LINE__, "(c_close): %s", db_strerror(i));
        eflag = true;
    }

    return eflag ? EX_ERROR : i;
}
...
bool
is_file_or_missing(const char *path)
...
static          u_int32_t
db_do_getpsize(DB * db)
{
    return get_psize(db, true);
}

static          u_int32_t
db_dofile(bfpath * bfp, u_int32_t(*func) (DB *))
{
    DB_ENV         *dbe = NULL;
    DB             *db;
    int             e;
    ...
    if (e != 0) {
        print_error(__FILE__, __LINE__, "error creating DB handle: %s", db_strerror(e));
        exit(EX_ERROR);
    }

    e = DB_OPEN(db, bfp, NULL, dbtype, DB_RDONLY | DB_NOMMAP, DS_MODE);
    if (e != 0) {
        print_error(__FILE__, __LINE__, "cannot open database %s: %s", bfp->filename, db_strerror(e));
        exit(EX_ERROR);
    }
    s = func(db);
    ...
    e = db->close(db, 0);
    if (e != 0) {
        print_error(__FILE__, __LINE__, "cannot close database %s: %s", bfp->filename, db_strerror(e));
        exit(EX_ERROR);
    }
    ...
    if (e != 0) {
        print_error(__FILE__, __LINE__, "cannot close environment %s: %s", bfp->dirname, db_strerror(e));
        exit(EX_ERROR);
    }
...
u_int32_t
db_pagesize(bfpath * bfp)
{
    return db_dofile(bfp, db_do_getpsize);
}

static          u_int32_t
db_do_leafpages(DB * dbp)
{
    int             ret;
    u_int32_t       c;
    DB_BTREE_STAT  *dbstat = NULL;
...
u_int32_t
db_leafpages(bfpath * bfp)
{
    return db_dofile(bfp, db_do_leafpages);
}

ex_t
db_verify(bfpath * bfp)
{
    DB_ENV         *dbe = NULL;
    DB             *db;
    int             e;
    ...
    if (e) {
        print_error(__FILE__, __LINE__, "database %s does not verify: %s", bfp->filename, db_strerror(e));
        exit(EX_ERROR);
    }
