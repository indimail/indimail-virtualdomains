...
The hacked source will also work with indimail by just changing the
...
e.g. mbhangui@indimail.org gets split into mbhangui as the user
and indimail.org as the domain. This split allows authentication against
...
====================
    o If installing from source:
      ...
      o ./configure --prefix=/usr --sysconfdir=/etc/indimail --mandir=/usr/share/man \
          --datarootdir=/usr/share/indimail \
          --libexecdir=/usr/libexec/indimail --enable-dlload-mysql=yes \
          --enable-mysqlprefix=/usr --libdir=/usr/[lib|lib64] \
    ...
    o Edit /etc/indimail/nssd.conf (or /var/vpopmail/etc/nssd.conf)
    ...
    o Start 'nssd' (e.g. "/usr/sbin/nssd" )

      For vpopmail, you need to have nssd run either by supervise or by your favourite method
      (rc, etc)
      ...
      # /usr/sbin/svctool --pwdlookup=/run/indimail/nssd.sock --threads=5 --timeout=-1 \
          --mysqlhost=localhost --mysqluser=indimail \
          --mysqlpass=ssh-1.5- --mysqlsocket=/var/run/mysqld/mysqld.sock \
          --servicedir=/service
    ...
    o Test pwlookup using a virtual user configured in indimail/vpopmail
