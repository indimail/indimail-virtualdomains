/*
 * $Log: evaluate.c,v $
 * Revision 1.1  2019-04-18 08:35:47+05:30  Cprogrammer
 * Initial revision
 *
...
#ifndef	lint
static char     sccsid[] = "$Id: evaluate.c,v 1.1 2019-04-18 08:35:47+05:30 Cprogrammer Exp mbhangui $";
#endif
...
int             same_str_len(char *a, char *b, int len);
...
evaluate(char *expr, struct val *result, struct vartable *vartable)
{
	...
	init_scantable();
	if ((mh = create_mem())) {
		if (expr && (str = (char *) mem_alloc(mh, str_len(expr) + 1))) {
			str_copy(str, expr);
			while (*str) {
 ...
 */
int
same_str_len(char *a, char *b, int len)
{
	if (!a || !b)
		return 0;	/*- false even if a == b == null */
	if (len == 0)
...
put_var(struct vartable *vt, char *name, struct val *value)
{
	...
	n = (char *) mem_alloc(vt->mh, str_len(name) + 1);
	if (v && n) {
		str_copy(n, name);
		v->name = n;
		v->val = *value;
