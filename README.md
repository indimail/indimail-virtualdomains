...
6. You can also set MYSQL\_PASS, PRIV\_PASS, ADMIN\_PASS environent variables to set your own passwords before running create\_services.
7. If you change the MySQL password for indimail after running create\_services, edit /etc/indimail/control/host.mysql. On FreeBSD/Darwin this file will be /usr/local/etc/indimail/control/host.mysql
8. [svctool](https://github.com/mbhangui/indimail-mta/wiki/svctool.8) requires credentials for MySQL user 'admin' for the mysqldump option. You can set credentials privileged users in /etc/indimail/.svctool.cnf and make sure only root user has read privileges on this file. You are encouraged to read [Setting MySQL Guide](https://github.com/mbhangui/indimail-mta/wiki/0-IndiMail-Wiki#setting-up-mysql), on how to save MySQL credentials.
